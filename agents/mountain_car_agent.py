...
class MountainCarQ:
    ...
    todo
    """
    def __init__(self, env: gym.Env, learning_rate: float = 0.01, discount_factor: float = 0.9, initial_epsilon: float = 1.0, final_epsilon: float = 0.1, epsilon_decay: float = 0.001, position_discretization: int = 1000, velocity_discretization: int = 100, action_discretization: int = 40):
        """
        todo
        """
        self.env = env
        ...
        self.possible_actions = np.linspace(ACTION_LEFT_BOUND, ACTION_RIGHT_BOUND, action_discretization)
        self.possible_positions = np.linspace(POSITION_LEFT_BOUND, POSITION_RIGHT_BOUND, position_discretization)
        self.possible_velocities = np.linspace(VELOCITY_LEFT_BOUND, VELOCITY_RIGHT_BOUND, velocity_discretization)
